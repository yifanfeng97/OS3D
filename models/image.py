...
class MVCNN(nn.Module):
    ...
    def forward(self, view_batch, global_ft=False, local_ft=False):
        n_view =  view_batch.size(1)
        view_batch = view_batch.view(
            -1, view_batch.size(2), view_batch.size(3), view_batch.size(4)
        )
        view_fts = self.ft_net(view_batch)
        local_view_fts = view_fts.view(-1, n_view, view_fts.size(-1))
